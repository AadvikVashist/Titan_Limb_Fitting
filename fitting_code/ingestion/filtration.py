    ...
    for key, value in dictionary.items():
        ...
        if len(value) == 1:
            ...
            for cube in value:
                ...
                # time.sleep(0.5)
            while True:
                inp = input("What cube do you want (give cube name)?")
                if inp == "bad" or inp == "no":
                    print("No input for this flyby")
                    break
                try:
                    try:
                        selections.append(value[indices.index(inp)])
                        break
                    except:
                        print("Invalid index, max is " + str(len(value) - 1))
                except:
                    print("Invalid input, must be a good string (don't include the C)")
        v = value
...
def get_existing_data_in_json_and_eliminate(data):
    ...
    for index, (flyby, cub) in enumerate(cubs.items()):
        ...
        flybys.append(flybyx)
    data = remove_ones_with_existing_flybys(data, flybys=flybys)
    data = get_vis_and_ir(data)
    data = get_info_cubes(data)
    data = get_filtered_phase(data, [0, 30])
    data = get_filtered_res(data, 20)
    data = filter_km_pixel(data, 300)
    data = select_best_per_flyby(data)
...
def run():
    ...
    refined_search = filter_dates(refined_search, "01-01-2012", "01-01-2023")
