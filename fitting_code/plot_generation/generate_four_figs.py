...
class gen_quad_plots:
    def __init__(self, devEnvironment: bool = True):
        if devEnvironment == True:
            self.save_dir = join_strings(
                SETTINGS["paths"]["parent_figures_path"], SETTINGS["paths"]["dev_figures_sub_path"],  SETTINGS["paths"]["figure_subpath"]["quadrant"])
        else:
            self.dps_dir = join_strings(
                SETTINGS["paths"]["parent_figures_path"], SETTINGS["paths"]["prod_figures_sub_path"],  "DPS_55")
            self.save_dir = join_strings(
                SETTINGS["paths"]["parent_figures_path"], SETTINGS["paths"]["prod_figures_sub_path"],  SETTINGS["paths"]["figure_subpath"]["quadrant"])
    ...
    def gen_cube_quads(self, data: dict, cube_index: int, cube_name: str = None):
        ...
        for plot_index, (wave_band, wave_data) in enumerate(data.items()):
            ...
            # self.get_imshow(cube_vis[plot_index-shift], rotation=data["meta"]["north_orientation_vis"])
            ...
            if plot_index-shift < 96:
                suffix = "_vis"
                axs[0].imshow(self.rotate_image(cube_vis[plot_index-shift],data["meta"]["north_orientation" + suffix]), cmap="gray")
                ...
                suffix = "_ir"
                axs[0].imshow(self.rotate_image(cube_ir[plot_index-96-shift],data["meta"]["north_orientation" + suffix]), cmap="gray")
            ...
            for pixel_index in north_slant["pixel_indices"]:
                pic[pixel_index[0], pixel_index[1]] = np.min(pic)
            for pixel_index in south_slant["pixel_indices"]:
                pic[pixel_index[0], pixel_index[1]] = np.min(pic)
                              
            shape = pic.shape
            rot_fig, rot_ax = plt.subplots()
            im = rot_ax.imshow(pic, cmap="gray")
            # rot_ax.plot([data["meta"]["center_of_cube" + suffix][1], data["meta"]["center_of_cube" + suffix][1] + np.sin(np.radians(data["meta"]["north_orientation" + suffix])) * 50],
            ...
            # rot_ax.plot([data["meta"]["center_of_cube" + suffix][1], data["meta"]["center_of_cube" + suffix][1] - np.sin(np.radians(data["meta"]["north_orientation" + suffix])) * 50],
            #             [data["meta"]["center_of_cube" + suffix][0], data["meta"]["center_of_cube" + suffix][0] + np.cos(np.radians(data["meta"]["north_orientation" + suffix])) * 50], color= south_color, label="South", linewidth=1)      

            rot_ax.scatter(data["meta"]["center_of_cube" + suffix][1], data["meta"]["center_of_cube" + suffix][0], color=center_color, s = 60 , label="Center of Disk")
            rot_ax.scatter(data["meta"]["lowest_inc_location" + suffix][1], data["meta"]["lowest_inc_location" + suffix][0], color=lowest_inc_color, s = 60 , label="Lowest Incidence Point")
            ...
            rot_ax.plot([data["meta"]["center_of_cube" + suffix][1], data["meta"]["center_of_cube" + suffix][1] + np.sin(np.radians(data["meta"]["north_orientation" + suffix] + north_slant["angle"])) * length],
            ...
            rot_ax.plot([data["meta"]["center_of_cube" + suffix][1], data["meta"]["center_of_cube" + suffix][1] + np.sin(np.radians(data["meta"]["north_orientation" + suffix] + south_slant["angle"])) * length],
            ...
            axs[1].plot([-1], [-1], color=north_slant_color, label="Northern Facing Slant", linestyle="dashed" ) 
            axs[1].plot([-1], [-1], color=south_slant_color, label="South Facing Slant", linestyle="dashed", )
            axs[1].scatter([-1], [-1], color=center_color, s = 60 , label="Center of Disk")
            axs[1].scatter([-1], [-1], color=lowest_inc_color, s = 60 , label="Lowest Incidence Point")

            rot_ax.set_xlim(0, shape[1]-1)
            rot_ax.set_ylim(shape[0]-1, 0)
            rot_ax.axis('off')  # Turn off the axis
            rotated_fig = self.rotate_fig(rot_fig,rot_ax, rotation=data["meta"]["north_orientation" + suffix])
            axs[1].imshow(rotated_fig)
            ...
            def get_best_corner(image):
                shape = image.shape
                topleft = np.mean(image[0:int(shape[0]/2),0:int(shape[1]/2)])
                topright = np.mean(image[0:int(shape[0]/2),int(shape[1]/2)::])
                bottomleft = np.mean(image[int(shape[0]/2)::,0:int(shape[1]/2)])
                bottomright = np.mean(image[int(shape[0]/2)::,int(shape[1]/2)::])
                return np.argmin([topleft,topright,bottomleft,bottomright])
            num = get_best_corner(rotated_fig)
            if num == 0:
                ...
                axs[1].legend(loc="lower right", framealpha = 0.9)
    ...
    def quad_all(self, multi_process: Union[bool,int] = False):
        data = self.get_data()
