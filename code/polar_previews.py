...
class polar_profile:
    ...
    def slanted_cross_section(self, cube, band, degree):
        ...
        ret = {"pixel_indices" : np.array(pixel_indices), "pixel_distances" : distances, "emission_angles" : emission_angles, "brightness_values" : brightness_values, "meta": {"actual_angle" : degree, "angle_rad" : angle_rad, "starting_x_y_image" : [start_x,start_y], "ending_x_y_image" : [endpoint_x, endpoint_y], "processing" : {"sorted" : False, "filtered" : False, "smoothed" : False, "interpolated" : False} }}
        return ret
    ...
    def complete_image_analysis_from_cube(
        ...
        # FIGURE GEN FOR PREVIEWS
        fig = plt.figure()
        plt.imshow(cube[band], cmap="gray")
        plt.title(str(band) + "/" + str(cube.w[band_index]) +
                " µm" + " in " + cube.img_id + " in " + cube.flyby.name)
        previews = {"original_image": fig}
        fig = plt.figure()
        img = cube[band].copy() 
        for index,degree in enumerate(degrees):
            fit = self.slanted_cross_section(cube, band, degree)
            plt.scatter(fit["pixel_indices"][:, 1],
                        fit["pixel_indices"][:, 0], marker="s", color = cmap(index / len(degrees) * 0.75))
            img[fit["pixel_indices"][:, 0],
                fit["pixel_indices"][:, 1]] = 0
        plt.imshow(img, cmap="gray")
        plt.title("Slant of " + str(band) + " in " + cube.img_id + " in " +
                    cube.flyby.name + " at all degrees")
        previews["slant_view"] = fig
        plt.close("all")
        return previews
...
class complete_cube:
    ...
    def analyze_dataset(self, cube_root: str, cube: str = None, force=False):
        ...
        for band in self.cube_vis.bands:
            ...
            data = self.run_analysis(self.cube_vis, north_orientation=north_orientation,
...
class preview_complete_dataset:
    ...
    def complete_dataset_preview(self):
        ...
        for cub in self.cubes:
            ...
            if cub + ".pkl" in os.listdir(fit_cube) and not force_write:
                ...
                print("skipping", cub, "because it already exists")
                continue
