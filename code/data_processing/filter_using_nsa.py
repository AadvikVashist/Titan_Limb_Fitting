...
class process_nsa_data_for_fitting:
    ...
    def try_to_remove_outliers(self, data, threshold = 2):
        ...
        if start is not None and end - start + 1 <= threshold:
            data[start:end+1] = False
        return data
    # def process_nsa(self, data : dict, cube_root: str, cube: str = None, force=False):

    #     leng = len(data)
    #     nsa_data = data["meta"]["nsa"]
    ...
    #     # lat_of_center_vis = data["meta"]["cube_vis"]["lat"][center_point[0],center_point[1]]
    #     # center_point = data["meta"]["center_of_cube_ir"]; center_point = (int(center_point[0]), int(center_point[1]))
    #     # lat_of_center_ir = data["meta"]["cube_ir"]["lat"][center_point[0],center_point[1]]


    #     # if nsa_latitude < 0:
    #     nsa_data_built_in_troubles = nsa_data["nsa_latitude"] 
    #     # + nsa_data["nsa_latitude"]/abs(nsa_data["nsa_latitude"]) *3 #1.25 is a magic number that works well
    #     # else:
    #         # nsa_data_built_in_troubles = nsa_data["nsa_latitude"] * 0.85 + nsa_data["nsa_latitude"]/abs(nsa_data["nsa_latitude"]) *1 #1.25 is a magic number that works well

    #     #this was the original idea, but then Aadvik realized that the Equator is not necessarily the same as the cube center (no matter how close it really is)
    #     # north_side = nsa_data["nsa_latitude"] >= 0 ; south_side = not north_side
    #     # cube_vis_mask = (data["meta"]["cube_vis"]["lat"]/nsa_data_built_in_troubles < 0) | (abs(data["meta"]["cube_vis"]["lat"]) > abs(nsa_data_built_in_troubles))
    #     # cube_ir_mask = (data["meta"]["cube_ir"]["lat"]/nsa_data_built_in_troubles < 0) | (abs(data["meta"]["cube_ir"]["lat"]) > abs(nsa_data_built_in_troubles))

    #     #this is the new idea, which is to use the cube center as the equator, but not the implementation
    #     # cube_vis_mask = abs(data["meta"]["cube_vis"]["lat"] - nsa_data_built_in_troubles) > 3
    #     # cube_ir_mask = abs(data["meta"]["cube_ir"]["lat"] - nsa_data_built_in_troubles) > 3
        
    #     #this is the implementation of the new idea

    #     # if lat_of_center_vis > nsa_data_built_in_troubles:
    #     #     cube_vis_mask = np.where((data["meta"]["cube_vis"]["lat"] > lat_of_center_vis) | (data["meta"]["cube_vis"]["lat"] < nsa_data_built_in_troubles), data["meta"]["cube_vis"]["lat"], False) # & (data["meta"]["cube_vis"]["lat"] < nsa_data_built_in_troubles)

    #     # else:
    #     #     cube_vis_mask = np.where((data["meta"]["cube_vis"]["lat"] < lat_of_center_vis) | (data["meta"]["cube_vis"]["lat"] > nsa_data_built_in_troubles), data["meta"]["cube_vis"]["lat"], False) # & (data["meta"]["cube_vis"]["lat"] < nsa_data_built_in_troubles)
        

    #     # Define your conditions for cube_ir_mask
    #     # if lat_of_center_ir < nsa_data_built_in_troubles:
    #     #     cube_ir_mask = np.where((data["meta"]["cube_ir"]["lat"] > lat_of_center_ir) & (data["meta"]["cube_ir"]["lat"] < nsa_data_built_in_troubles), data["meta"]["cube_vis"]["lat"], False)
    #     # else:
    #     #     cube_ir_mask = np.where((data["meta"]["cube_ir"]["lat"] < lat_of_center_ir) | (data["meta"]["cube_ir"]["lat"] > nsa_data_built_in_troubles), data["meta"]["cube_vis"]["lat"], False)
    #     #TAKE A LOOK AT THE COMMENTED PLOT TO UNDERSTAND THE NSA CALCULATIONS
        
        
    #     # shows = np.where(cube_vis_mask, data["meta"]["cube_vis"]["lat"],False)

    #     for index, (wave_band, wave_data) in enumerate(data.items()):
    #         if "µm_" not in wave_band:
    #             continue
    #         # plt.imshow(cube_vis_mask)
    #         count = 0
    #         for slant, slant_data in wave_data.items():
    #             slant_angle = slant
    #             pixel_indices = np.array(slant_data["pixel_indices"])
    #             indices_mask = np.array([True if abs(data["meta"]["cube_vis"]["lat"][pixel[0], pixel[1]] - nsa_data_built_in_troubles) > 1 else False for pixel in pixel_indices])
                
    #             # indices_mask = np.where(cube_vis_mask[pixel_indices[:,0], pixel_indices[:,1]], True, False)
                
                
    #             # indices_mask = self.try_to_remove_outliers(indices_mask)
    #             # if len(indices_mask) == 0 or not any(indices_mask):
    #             #     if index < 96:
    #             #         mask = np.where(abs(data["meta"]["cube_vis"]["lat"] - nsa_data_built_in_troubles) > 1, data["meta"]["cube_vis"]["bands"][index] *2, data["meta"]["cube_vis"]["bands"][index])
    #             #         mask = np.where(data["meta"]["cube_vis"]["ground"], mask, data["meta"]["cube_vis"]["lat"]/ 900)
    #             #     else:
    #             #         mask = np.where(abs(data["meta"]["cube_ir"]["lat"] - nsa_data_built_in_troubles) > 1, data["meta"]["cube_ir"]["bands"][index - 96] *2, data["meta"]["cube_ir"]["bands"][index - 96])
    #             #         mask = np.where(data["meta"]["cube_ir"]["ground"], mask, data["meta"]["cube_ir"]["lat"]/ 900)
    ...
    #             data[wave_band][slant]["pixel_indices"] = pixel_indices[indices_mask]
    ...
    def process_nsa(self, data : dict, cube_root: str, cube: str = None, force=False):
        ...
        for index, (wave_band, wave_data) in enumerate(data.items()):
            ...
            for slant, slant_data in wave_data.items():
                ...
                indices_mask = np.array([emission_angle > 25 for emission_angle in emission])
                # indices_mask = np.where(cube_vis_mask[pixel_indices[:,0], pixel_indices[:,1]], True, False)
    ...
    def select_nsa_data_in_all(self):
        ...
        for index, (cube_name, cube_data) in enumerate(data.items()):      
            ...
            data[cube_name] = self.process_nsa(cube_data, cube_root=join_strings(self.cubes_location, cube_name), cube=cube_name, force=force_write)
        ...
        if (os.path.exists(join_strings(self.save_dir, get_cumulative_filename("nsa_filtered_out_sub_path"))) and appended_data):
            ...
            check_if_exists_or_write(join_strings(self.save_dir, get_cumulative_filename("nsa_filtered_out_sub_path")), data = data, save=True, force_write=True)
        elif force_write:
            check_if_exists_or_write(join_strings(self.save_dir,get_cumulative_filename("nsa_filtered_out_sub_path")), data = data, save=True, force_write=True)
        else:
